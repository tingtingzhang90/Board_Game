...
def alphabeta(board, depth, alpha, beta, maximizingPlayer):
    ...
    if maximizingPlayer:
        ...
        for col in valid_locations:
            ...
            b_copy = board.copy()
            drop_piece(b_copy, row, col, AI_PIECE)
            _, new_score = alphabeta(b_copy, depth - 1, alpha, beta, False)
            if new_score >= value:
                value = new_score
                column = col
        ...
        for col in valid_locations:
            ...
            b_copy = board.copy()
            drop_piece(b_copy, row, col, PLAYER_PIECE)
            _, new_score = alphabeta(b_copy, depth - 1, alpha, beta, True)
            if new_score < value:
                value = new_score
                column = col
            beta = min(beta, value)
...
def pick_best_movie(board, piece):
    best_score = 0
    valid_locations = get_valid_location(board)
...
while not game_over:
    ...
    if turn == AI and not game_over:
        ...
        # col = pick_best_movie(board, AI_PIECE)
